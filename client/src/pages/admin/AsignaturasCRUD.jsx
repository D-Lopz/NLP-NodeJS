...
import { getAsignaturas, crearAsignatura, actualizarAsignatura, eliminarAsignatura } from "../../api/api";

const EMPTY = { nombre_asignatura: "", creditos: "", nombre_docente: "" };

function Modal({ asignatura, onClose, onSave }) {
      ...
      <div className="w-full max-w-md rounded-2xl border border-white/12 p-6" style={{ background: "#1c1c1c" }}>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <input value={form.nombre_docente} placeholder="Nombre del docente"
              onChange={(e) => setForm({ ...form, nombre_docente: e.target.value })}
              className="w-full rounded-xl border border-white/10 bg-white/4 text-white px-4 py-2.5 text-sm focus:outline-none focus:border-violet-500/60 transition-all placeholder:text-white/20" />
          </div>
...
function AsignaturasCRUD() {
  ...
  const cargar = () => {
    setLoading(true);
    getAsignaturas().then((d) => setAsignaturas(d.asignaturas || d || []))
  ...
  return (
    <div className="p-8 max-w-4xl">
      {modal !== null && (
        <Modal asignatura={modal === "new" ? null : modal}
