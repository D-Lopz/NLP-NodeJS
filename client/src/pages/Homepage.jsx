...
function StatCard({ label, value, accent, icon, loading }) {
  return (
    <div
      className="rounded-xl border border-white/8 p-5 flex flex-col gap-3 relative overflow-hidden"
    ...
    <div className="min-h-screen pt-24 pb-12 px-6 max-w-5xl mx-auto">
      ...
      <div className="mb-10">
        ...
        <h1 className="text-3xl font-bold text-white">
          Evaluación docente <span className="text-white/30">con NLP</span>
        </h1>
        <p className="text-white/40 mt-1 text-sm">Análisis automático de sentimientos en comentarios estudiantiles.</p>
      </div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <Link to="/evaluar" className="text-xs text-violet-400 hover:text-violet-300 transition-colors font-mono">
          + Nueva evaluación
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-white/30 text-sm">
          Aún no hay evaluaciones. <Link to="/evaluar" className="text-violet-400 hover:underline">Crea la primera</Link>
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {recientes.map((ev) => (
            <div
              key={ev._id}
              className="rounded-xl border border-white/8 p-4 flex items-start gap-4 hover:border-white/15 transition-all duration-200"
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  <span className="text-sm font-semibold text-white">{ev.docente?.nombre ?? "Docente"}</span>
                ...
                {ev.nlp.razon && (
                  <p className="text-xs text-white/30 italic mt-0.5">"{ev.nlp.razon}"</p>
                )}
              </div>
