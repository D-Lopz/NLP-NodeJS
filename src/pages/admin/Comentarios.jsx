...
function Comentarios() {
  ...
  const filtradas = filtroDocente
    ? evaluaciones.filter((e) => e.docente?._id?.toString() === filtroDocente)
    : evaluaciones;

  const docenteFiltrado = docentes.find(d => d._id === filtroDocente);

  // Nombre limpio del docente (sin dash)
  const nombreDocente = (d) =>
        ...
        <div className="rounded-xl border border-violet-500/25 bg-violet-500/5 p-4 mb-6 flex items-center gap-4">
          <div className="flex-1">
            ...
            {docenteFiltrado.docente?.titulo && (
              <p className="text-xs text-white/40 mt-0.5">{docenteFiltrado.docente.titulo}</p>
            )}
          </div>
          <div className="flex gap-4 text-xs font-mono">
            ...
            <div className="text-center">
              <p className="text-amber-400">{filtradas.filter(e=>e.nlp?.sentiment==="neutro").length}</p>
            ...
            <div key={ev._id} className="rounded-xl border border-white/8 p-4 hover:border-white/15 transition-all"
              ...
              <div className="flex items-start justify-between gap-4 mb-2">
                <div className="flex items-center gap-2 flex-wrap">
                  {/* Nombre del docente si no hay filtro activo */}
