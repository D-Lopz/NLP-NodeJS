...
import { getDocentes, crearEvaluacion, getEvaluaciones } from "../../api/api";
...
function Evaluar() {
  ...
  useEffect(() => {
    // Cargar docentes y evaluaciones ya hechas en paralelo
    Promise.all([getDocentes(), getEvaluaciones()])
      .then(([docs, evals]) => {
        // IDs de docentes que ya fueron evaluados
        const yaEvaluados = new Set(
          evals.map(e => e.docente?._id?.toString())
        );
        // Filtrar docentes que aún no han sido evaluados
        const disponibles = docs.filter(d => !yaEvaluados.has(d._id.toString()));
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setResult(data);
      // Quitar el docente evaluado de la lista
            ...
            <div>
              ...
              {loadingDocs ? (
                ...
                <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-amber-400 text-sm">
                  Ya evaluaste a todos los docentes disponibles.
                </div>
              ) : (
                <select value={docenteId} onChange={(e) => setDocenteId(e.target.value)}
                  ...
                  {docentes.map((d) => (
                    <option key={d._id} value={d._id} className="bg-zinc-900">
                      {d.nombre}{d.docente?.titulo ? ` — ${d.docente.titulo}` : ""}
                    </option>
                  ))}
                </select>
              )}
